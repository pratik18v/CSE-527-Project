...
depth = int(sys.argv[2]) if len(sys.argv) > 2 else 28    #changed from 18
width = int(sys.argv[3]) if len(sys.argv) > 3 else 10    #changed from 1
...
f = gzip.open('data/weights/%s.pklz'%(variant_name), 'wb')
pickle.dump(best_params, f)
f.close()

#Plot network layers
#plot_conv_weights(output_layer.layers_[2], figsize=(4, 4))
#plt.savefig("convweights.png")
...
pyplot.legend(loc=1)
pyplot.savefig('plots/%s.png'%(variant_name))
pyplot.clf()
#pyplot.show()
